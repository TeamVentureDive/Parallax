document.addEventListener('DOMContentLoaded', () => {
    ...
    tokenFields.forEach((field, index) => {
      field.addEventListener('input', (event) => {
        ...
        if (!input.value && previousInput && (event.inputType === 'deleteContentBackward' || event.keyCode === 8)) {
          // Move focus to previous field when backspace is pressed and current field is empty
          previousInput.focus();
          previousInput.setSelectionRange(previousInput.value.length, previousInput.value.length);
        } else if (input.value && nextInput && input.value.length >= input.maxLength) {
          // Move focus to next field when current field is filled and has reached its maximum length
          nextInput.focus();
        } else if (input.value && nextInput && (event.inputType !== 'deleteContentBackward' && event.keyCode !== 8) || (event.inputType === 'insertText' && input.value.length >= input.maxLength)) {
        ...
        console.log(token);
        if (token.length === 7) { // Wenn focus bug gefixxt is funktioneirt das. Jetzt muss man hier 6 eingeben. 
